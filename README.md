...
This has commits from contributors to master before I took a deep breath and renamed branches.  The pyspf-2.1 branch is 
an earlier snapshot.  The goal of this branch is to merge fixes from master (now pyspf-2.0) with new ideas for pyspf-2.1.

 1. Use a directory for the module.  
 2. use dnsplug, dnsswitch, or other separate module, perhaps under spf directory for selecting dns library.  dkimpy also 
 needs the pluggable DNS support.
 3. modules for ip6 support on ancient python system - probably not terribly relevant anymore.
